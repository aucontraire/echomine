...
- **Model Provenance**: Surface LLM model identifier on messages and conversations
  - `Message.model: str | None` — the model that produced this message (e.g., `gpt-5`, `o3`)
  - `Conversation.models_used: list[str]` — distinct, ordered list of models seen across assistant messages
  - OpenAI adapter: reads per-message `metadata.model_slug`, falls back to `default_model_slug`
  - Claude adapter: `None` / empty (Anthropic export does not include model info)
  - Both fields are additive with safe defaults — fully backward-compatible
...
[1.5.0]: https://github.com/aucontraire/echomine/compare/v1.4.0...v1.5.0
